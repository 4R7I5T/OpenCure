...
Entrez.email = os.getenv("NCBI_EMAIL", "your_registered_email")
Entrez.api_key = os.getenv("NCBI_API_KEY", "your_api_key")
...
def write_fasta(sequences, sequence_ids, filename="sequences.fasta"):
...
required_columns = {"Chromosome", "Position", "Variant_Type", "Reference", "Alternate"}
...
def process_row(index, row):
    ...
    ref_allele = reference.strip().upper()
    if center_index < 0 or center_index + len(ref_allele) > len(cleaned_sequence):
        logging.error(f"Reference position out of range for {chromosome}:{position}")
        return (index, None, f"sequence_{index}")
    seq_ref = cleaned_sequence[center_index:center_index + len(ref_allele)]
    if seq_ref != ref_allele:
        logging.error(f"Reference allele mismatch for {chromosome}:{position}: expected {ref_allele}, found {seq_ref}")
        return (index, None, f"sequence_{index}")
...
write_fasta(sequences, sequence_ids)
...
gRNAs = parse_flashfry_results(results_file=os.path.join("flashfry_output", "results.tsv"))
...
try:
    ...
    tools.display_dataframe_to_user(name="Updated Variant Data with gRNA Design", dataframe=data)
except ImportError:
    pass