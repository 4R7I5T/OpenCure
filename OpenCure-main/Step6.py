...
input_file_path = "./output_annotated.csv"
output_dir = "./samples"
...
for chunk in pd.read_csv(input_file_path, chunksize=chunk_size):
    required_columns = {"chromosome", "on_site_score"}
    missing_columns = required_columns - set(chunk.columns)
    if missing_columns:
        raise ValueError(f"Missing required columns in {input_file_path}: {missing_columns}")

    score_column = "combined_score" if "combined_score" in chunk.columns else "on_site_score"
    chunk[score_column] = pd.to_numeric(chunk[score_column], errors='coerce')

    for index, row in chunk.iterrows():
        sequence_id = row['chromosome']
        ...
        # Append the row as a tuple (score, row_data)
        sequence_data[sequence_id].append((score, row))
