...
def load_crispr_data(csv_file):
    try:
        ...
        required_columns = {"crispr_id", "sequence", "long_sequence", "start_pos", "end_pos", "cutsite"}
        missing_columns = required_columns - set(crispr_df.columns)
        if missing_columns:
...
def generate_input_output_scenarios(crispr_df, sequence):
    ...
    for _, row in crispr_df.iterrows():
        try:
            ...
            if start_pos < 1 or start_pos + 99 > len(sequence):
                print(f"Skipping entry due to out-of-range start_pos: {start_pos}")
                continue
